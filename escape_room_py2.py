...
class Right(Office):
    ...
    def access_safe(self):
        ...
        if safe_state:
            ...
            user_permission = raw_input("Do you want to try?(y/n)\n> ")
            if user_permission == 'y':
                while True:
                    enter_code = raw_input("[code]> ")
                    if enter_code == code:
    ...
    def access_circuit_breaker(self):
        ...
        if circuit_breaker_state:
            ...
            try_order = raw_input("Would you like to try fixing it?(y/n)\n> ")
            if try_order == 'y':
...
class Actions(object):
    ...
    def use(self, thing1, thing2):
        ...
        if thing1 in inventory:
            if thing2 in self.objects.keys():
                try:
